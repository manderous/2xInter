...
class FSLTrainer_bart_contra(torch.nn.Module):
    ...
    def do_train(self):
        ...
        print('Optimizer: SGD')
        # optimizer = torch.optim.SGD(params, lr=self.args.lr)  # rams, ace
        # 默认值：momentum = 0.9, weight_decay = 0.0005
        optimizer = torch.optim.SGD(params, lr=self.args.lr, momentum=0.9, weight_decay=0.0007)  # 使用Simsiam的参数, lowkbp
        ...
        for i, batch in enumerate(self.train_dl):
            ...
            # return_item, pos_logits, loss_contra = self.fsl_model(batch, self.train_setting, self.template_ll_list)
            ...
            #  pos样本的loss
            # pos_loss = 0.0
            # for pos_logit in pos_logits:  # 如果没有计算pos_logits，以下这两行需要注释掉
            #     pos_loss += self.ce(pos_logit, target)
            # pos_loss = pos_loss / len(pos_logits)
            #  pos样本的loss
            # total_loss = total_loss + loss + pos_loss + loss_contra  # few-shot loss 和 contrastive loss
            total_loss = total_loss + loss + loss_contra  # few-shot loss 和 contrastive loss
    ...
    def do_eval(self, part='test', iteration=0):
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(dl):
                ...
                # return_item, pos_logits_all, loss_contra = self.fsl_model(batch, self.eval_setting, self.template_ll_list)
                logits = return_item.view(-1, N)
        ...
        encoder_way_shot_name=f"{self.args.encoder}_{self.args.PI_num}_{self.args.way}_{self.args.shot}"
