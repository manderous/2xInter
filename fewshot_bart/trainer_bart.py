...
class FSLTrainer_bart(torch.nn.Module):
    ...
    def do_train(self):
        ...
        for i, batch in enumerate(self.train_dl):
            ...
            return_item = self.fsl_model(batch, self.train_setting, self.template_ll_list)
    ...
    def do_eval(self, part='test', iteration=0):
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(dl):
                for k, v in batch.items():
                    if k not in self.ignore_cuda_feature:
                        batch[k] = batch[k].cuda()
                return_item = self.fsl_model(batch, self.eval_setting, self.template_ll_list)
                logits = return_item.view(-1, N)
        ...
        if "lowkbp" in self.args.dataset:
            path = 'checkpoints/lowkbp/{}/{}/{}.{}.{}.json'.format(self.args.dataset, encoder_way_shot_name,
                                                                   self.args.ex, part, iteration)
        elif self.args.dataset == "rams" or self.args.dataset == "ace":
            path = 'checkpoints/{}/{}/{}.{}.{}.json'.format(self.args.dataset, encoder_way_shot_name, self.args.ex,
                                                            part, iteration)
