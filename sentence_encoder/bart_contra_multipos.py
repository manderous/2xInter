...
class Bart_contra_multipos(BartPretrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
        self.PI_num=config.PI_num
    ...
    def forward(self, B, N, K, Q, batch):
        ...
        # N = 3
        # enc_input_ids = enc_input_ids[:27]
        # enc_mask_ids = enc_mask_ids[:27]

        # Prompt intervention
        pos_decoder_context = []  # 收集所有正样本的表示
        for pos_id in range(3):  # 1个锚点样本，2个正样本
            ...
            if pos_id == 0:
                decoder_context = context_outputs.encoder_last_hidden_state  # 锚点表示
            else:
                pos_decoder_context.append(context_outputs.encoder_last_hidden_state)  # 正样本表示

        # pos_logits = [0.0, 0.0]
        pos_logits = []
        for pos_id in range(len(pos_decoder_context)):  # 对于2个正样本，获得它们的最终表示
            dec_prompt_ids = batch['dec_prompt_ids_0']  # 选择第0个提示模板
            dec_prompt_mask_ids = batch['dec_prompt_mask_ids_0']  # 选择第0个提示模板
            mask_index = batch['mask_index_0']  # 选择第0个提示模板

            # dec_prompt_ids = dec_prompt_ids[:27]  # 选择第0个提示模板
            # dec_prompt_mask_ids = dec_prompt_mask_ids[:27]  # 选择第0个提示模板
            # mask_index = mask_index[:27]  # 选择第0个提示模板
            ...
            pos_logits.append(torch.squeeze(logits, 0))

        # Instance intervention
        all_logits = []
        for tem_id in range(self.PI_num):  # 3,4,5,6：遍历所有Prompt，1：仅考虑一个Prompt，即去除因果干预
            ...
            # # 获取第0次循环的原型和索引，用于后面的contrastive loss
            # if tem_id == 0:
